...
static LedJobQueue _led_job_queue = {NULL, JOB_QUEUE_SIZE, JOB_QUEUE_SIZE, {}, PTHREAD_MUTEX_INITIALIZER};
...
static LedJob get_next_led_job() {
    ...
    if (_led_job_queue.start != NULL && _led_job_queue.start->job_type != LED_JOB_NONE) {
        next_job = *_led_job_queue.start;
        
        _led_job_queue.start->job_type = LED_JOB_NONE;

        if (_led_job_queue.start == (&_led_job_queue.queue[_led_job_queue.capacity - 1])) {
